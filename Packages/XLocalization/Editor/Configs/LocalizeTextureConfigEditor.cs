...
{
    [CustomEditor(typeof(LocalizeTextureConfig))]
    public class LocalizeTextureConfigEditor : UnityEditor.Editor
    {
        private LocalizeTextureConfig config;
        ...
        {
            config = (LocalizeTextureConfig)target;
        ...
        {
            var assetDatas = config.LanguageConfig.Languages.Select(language => new LocalizeAssetsData<Texture>() { Language = language }).ToList();
            var data = new LocalizeAssetTableData<Texture>()
            {
                Key = "",
        ...
        private void RefreshLanguage()
        {
            ...
            foreach (var language in languages)
            {
                foreach (var td in config.TableDatas)
                {
                    var data = new LocalizeAssetsData<Texture>()
                    {
                        Language = language
                    };
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("导入CSV", GUILayout.Width(width/2)))
            {
                ...
                {
                    var tableDatas = datas.ToLocalizeStringTableData<Texture>();
            ...
            EditorGUILayout.LabelField("本地化数据(Texture)", EditorStyles.boldLabel);
        ...
        private void DrawTable(List<LocalizeAssetTableData<Texture>> data)
        {
            // ---------- 数据行 ----------
        ...
        private void DrawRow(string Key, List<Texture> cs, int index)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            for (var i = 0; i < cs.Count; i++)
            {
                ...
                config.TableDatas[index].AssetDatas[i].Asset = (Texture)EditorGUI.ObjectField(contentRect, cs[i], typeof(Texture), false);
                x += CellWidth;
