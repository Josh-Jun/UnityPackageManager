...
{
    [CustomEditor(typeof(LocalizeGameObjectConfig))]
    public class LocalizeGameObjectConfigEditor : UnityEditor.Editor
    {
        private LocalizeGameObjectConfig config;
        ...
        {
            config = (LocalizeGameObjectConfig)target;
        ...
        {
            var assetDatas = config.LanguageConfig.Languages.Select(language => new LocalizeAssetsData<GameObject>() { Language = language }).ToList();
            var data = new LocalizeAssetTableData<GameObject>()
            {
                Key = "",
        ...
        private void RefreshLanguage()
        {
            ...
            foreach (var language in languages)
            {
                foreach (var td in config.TableDatas)
                {
                    var data = new LocalizeAssetsData<GameObject>()
                    {
                        Language = language
                    };
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("导入CSV", GUILayout.Width(width/2)))
            {
                ...
                {
                    var tableDatas = datas.ToLocalizeStringTableData<GameObject>();
            ...
            EditorGUILayout.LabelField("本地化数据(GameObject)", EditorStyles.boldLabel);
        ...
        private void DrawTable(List<LocalizeAssetTableData<GameObject>> data)
        {
            // ---------- 数据行 ----------
        ...
        private void DrawRow(string Key, List<GameObject> cs, int index)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            for (var i = 0; i < cs.Count; i++)
            {
                ...
                config.TableDatas[index].AssetDatas[i].Asset = (GameObject)EditorGUI.ObjectField(contentRect, cs[i], typeof(GameObject), false);
                x += CellWidth;
