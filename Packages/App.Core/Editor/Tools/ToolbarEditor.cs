...
namespace App.Editor.Tools
{
    public static class ToolbarEditor
    {
        ...
        public static void InitializeOnLoad()
        {
            ...
            if (!Directory.Exists($"{Directory.GetParent(Application.dataPath)}/Tools"))
            {
                UnCompressionTools();
            }
        }
        ...
        {
            {
                ...
                new ToolbarToggleItem
                {
                    ...
                    imageOn = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/auto_play_on.png"),
                    imageOff = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/auto_play_off.png"),
            ...
            },
            {
                ...
                new ToolbarToggleItem
                {
                    ...
                    imageOn = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/restore_view_on.png"),
                    imageOff = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/restore_view_off.png"),
        ...
        private static readonly List<ToolbarButtonItem> ToolbarButtonItems = new ()
        {
            ...
            {
                image = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/file_path_refresh.png"),
            ...
            new ToolbarButtonItem
            {
                image = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/keystore.png"),
                tooltip = "Update Keystore Setting",
                eventHandler = MenuToolsEditor.UpdateKeystore,
            },
            new ToolbarButtonItem
            {
                image = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/copy_csharp.png"),
                tooltip = "Copy TemplateScripts",
                eventHandler = MenuToolsEditor.UpdateKeystore,
            },
            new ToolbarButtonItem
            {
                image = AssetDatabase.LoadAssetAtPath<Texture>($"{EditorHelper.BaseEditorPath}/Tools/Images/app_settings.png"),
        ...
        private static void UnCompressionTools()
        {
            ...
            try
            {
                ...
                Debug.Log($"extractPath: {extractPath}");
                if (!Directory.Exists(extractPath))
                    Directory.CreateDirectory(extractPath);

                using (var fsIn = new FileStream(zipFilePath, FileMode.Open, FileAccess.Read))
                {
                    using (var zipStream = new ZipInputStream(fsIn))
                    {
                        while (zipStream.GetNextEntry() is { } entry)
                        {
                            var entryFileName = Path.Combine(extractPath, entry.FileName);
                            ...
                            using var fsOut = File.Create(entryFileName);
