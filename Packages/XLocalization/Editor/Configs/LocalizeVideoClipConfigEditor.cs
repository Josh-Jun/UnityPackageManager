...
{
    [CustomEditor(typeof(LocalizeVideoClipConfig))]
    public class LocalizeVideoClipConfigEditor : UnityEditor.Editor
    {
        private LocalizeVideoClipConfig config;
        ...
        {
            config = (LocalizeVideoClipConfig)target;
        ...
        {
            var assetDatas = config.LanguageConfig.Languages.Select(language => new LocalizeAssetsData<VideoClip>() { Language = language }).ToList();
            var data = new LocalizeAssetTableData<VideoClip>()
            {
                Key = "",
        ...
        private void RefreshLanguage()
        {
            ...
            foreach (var language in languages)
            {
                foreach (var td in config.TableDatas)
                {
                    var data = new LocalizeAssetsData<VideoClip>()
                    {
                        Language = language
                    };
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("导入CSV", GUILayout.Width(width/2)))
            {
                ...
                {
                    var tableDatas = datas.ToLocalizeStringTableData<VideoClip>();
            ...
            EditorGUILayout.LabelField("本地化数据(VideoClip)", EditorStyles.boldLabel);
        ...
        private void DrawTable(List<LocalizeAssetTableData<VideoClip>> data)
        {
            // ---------- 数据行 ----------
        ...
        private void DrawRow(string Key, List<VideoClip> cs, int index)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            for (var i = 0; i < cs.Count; i++)
            {
                ...
                config.TableDatas[index].AssetDatas[i].Asset = (VideoClip)EditorGUI.ObjectField(contentRect, cs[i], typeof(VideoClip), false);
                x += CellWidth;
