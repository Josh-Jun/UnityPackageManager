...
{
    [CustomEditor(typeof(LocalizeSpriteConfig))]
    public class LocalizeSpriteConfigEditor : UnityEditor.Editor
    {
        private LocalizeSpriteConfig config;
        ...
        {
            config = (LocalizeSpriteConfig)target;
        ...
        {
            var assetDatas = config.LanguageConfig.Languages.Select(language => new LocalizeAssetsData<Sprite>() { Language = language }).ToList();
            var data = new LocalizeAssetTableData<Sprite>()
            {
                Key = "",
        ...
        private void RefreshLanguage()
        {
            ...
            foreach (var language in languages)
            {
                foreach (var td in config.TableDatas)
                {
                    var data = new LocalizeAssetsData<Sprite>()
                    {
                        Language = language
                    };
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("导入CSV", GUILayout.Width(width/2)))
            {
                ...
                {
                    var tableDatas = datas.ToLocalizeStringTableData<Sprite>();
            ...
            EditorGUILayout.LabelField("本地化数据(Sprite)", EditorStyles.boldLabel);
        ...
        private void DrawTable(List<LocalizeAssetTableData<Sprite>> data)
        {
            // ---------- 数据行 ----------
        ...
        private void DrawRow(string Key, List<Sprite> cs, int index)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            for (var i = 0; i < cs.Count; i++)
            {
                ...
                config.TableDatas[index].AssetDatas[i].Asset = (Sprite)EditorGUI.ObjectField(contentRect, cs[i], typeof(Sprite), false);
                x += CellWidth;
