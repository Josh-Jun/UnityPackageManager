...
{
    [CustomEditor(typeof(LocalizeStringConfig))]
    public class LocalizeStringConfigEditor : UnityEditor.Editor
    {
        private LocalizeStringConfig config;
        ...
        {
            config = (LocalizeStringConfig)target;
        ...
        {
            var stringDatas = config.LanguageConfig.Languages.Select(language => new LocalizeStringData() { Language = language }).ToList();
            var data = new LocalizeStringTableData()
            {
                Key = "",
        ...
        private void RefreshLanguage()
        {
            ...
            {
                foreach (var sd in td.StringDatas.ToList().Where(sd => !config.LanguageConfig.Languages.Contains(sd.Language)))
                {
                    td.StringDatas.Remove(sd);
                }
            }

            // 添加语言表数据
            var languages = config.LanguageConfig.Languages.ToList();
            foreach (var sd in config.TableDatas.SelectMany(td => td.StringDatas.ToList().Where(sd => languages.Contains(sd.Language))))
            {
                languages.Remove(sd.Language);
            }
            ...
            foreach (var language in languages)
            {
                foreach (var td in config.TableDatas)
                {
                    var data = new LocalizeStringData()
                    {
                        Language = language
                    };
                    td.StringDatas.Add(data);
                }
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("导入CSV", GUILayout.Width(width/2)))
            {
                ...
                {
                    var tableDatas = datas.ToLocalizeStringTableData();
            ...
            EditorGUILayout.LabelField("本地化数据(String)", EditorStyles.boldLabel);
        ...
        private void DrawTable(List<LocalizeStringTableData> data)
        {
            // ---------- 数据行 ----------
            for (var i = 0; i < data.Count; i++)
            {
                DrawLine((config.LanguageConfig.Languages.Count + 1) * CellWidth + 1,1, Color.gray);
                var cells = data[i].StringDatas.Select(t => t.Value).ToList();
        ...
        private void DrawRow(string Key, List<string> cs, int index)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            for (var i = 0; i < cs.Count; i++)
            {
                ...
                config.TableDatas[index].StringDatas[i].Value = GUI.TextField(contentRect, cs[i]);
                x += CellWidth;
